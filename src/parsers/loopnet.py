...
def parse_loopnet(email):
    subj = (email.get("subject") or "").lower()
    froms = email.get("from") or []
    # Only attempt if LoopNet + mentions business/listing in subject or sender
    if ("loopnet" not in subj and not any("loopnet" in (x[1] or "").lower() for x in froms)):
        return []
    if not any(k in subj for k in ("business", "listing", "saved", "alert")):
        # be conservative to avoid real-estate-only alerts
        return []
    ...
    a = soup.find("a", string=re.compile(r"(View|See)\s+(Business|Listing|Details)", re.I))
