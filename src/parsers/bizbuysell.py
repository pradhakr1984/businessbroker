# Example site-specific parser (adjust to the actual emails you receive).
import re
from bs4 import BeautifulSoup

def parse_bizbuysell(email):
    subj = (email.get("subject") or "").lower()
    froms = email.get("from") or []
    if "bizbuysell" not in subj and not any("bizbuysell" in (x[1] or "").lower() for x in froms):
        return []

    html = email.get("text_html") or ""
    if not html:
        return []
    ...
    a = soup.find("a", string=re.compile(r"View Listing|See Listing|View\s+Details", re.I))
